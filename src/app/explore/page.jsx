...
const ExplorePage = () => {
    ...
    const [currentPageFacilities, setCurrentPageFacilities] = useState([]); // New state for current page data
    ...
    const { data: facilities, isLoading, error, lastSnapDoc } = useFacilities({
    ...
    // Handle data for current page
    useEffect(() => {
        if (facilities) {
            setCurrentPageFacilities(facilities);
        }
    }, [facilities]);

    // Filter facilities based on criteria
    const filteredFacilities = useMemo(() => {
        if (!currentPageFacilities) return [];

        return currentPageFacilities.filter(facility => {
            // Search query filter
            if (searchQuery && !facility.name.toLowerCase().includes(searchQuery.toLowerCase())) {
                return false;
            }
    ...
    }, [currentPageFacilities, searchQuery, priceRange, selectedSports, minRating]);

    // Pagination handlers
    const handleNextPage = () => {
        if (lastSnapDoc && facilities?.length === itemsPerPage) {
    ...
    const hasMore = facilities?.length === itemsPerPage;
            ...
            <div className="max-w-7xl mx-auto px-4 py-25">
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="flex-1">
                        ...
                        {/* Facilities Grid */}
                        {filteredFacilities.length === 0 && !isLoading && lastSnapDocList.length === 0 ? (
                            <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                                <MapPin className="mx-auto h-16 w-16 text-gray-300 mb-4" />
                                <h3 className="text-xl font-bold text-gray-900 mb-2">No facilities found</h3>
                                <p className="text-gray-600 mb-6">Try adjusting your filters or search terms</p>
                            ...
                            <>
                                ...
                                <div className="mt-12 flex flex-col sm:flex-row justify-between items-center gap-4">
                                    <Button
                                        ...
                                        variant="outline"
                                        className="border-theme-purple text-theme-purple hover:bg-theme-purple/10 min-w-[120px]"
                                    >
                                        Previous
                                    </Button>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <select
                                            className="bg-theme-purple/10 rounded-lg outline-none p-2 text-sm border border-theme-purple/20 focus:ring-2 focus:ring-theme-purple"
                                            value={itemsPerPage}
